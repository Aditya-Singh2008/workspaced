...
- **Linux** renders through webkit2gtk, which trails Chromium and Safari on some CSS/JS features and varies by distro. Canvas-heavy rendering (PDF and image display are the core of this app) is the most likely place for inconsistencies to surface. Test on at least one Linux distro during Phase 3 and Phase 4, not only at the end.
  - **The floor is webkit2gtk 2.44** (Safari 17.4, Chromium 119, Ubuntu 24.04 LTS), set by `Promise.withResolvers` in pdf.js. Anything added later that needs a newer baseline is a decision, not a detail — see the pin above.
...
4. **The macOS PDF hang: the fix no longer depends on which WKWebView theory is
   right, but the mode it lands in is still unmeasured.** Every PDF hung on "loading"
   in the macOS bundle. The first fix was built on the URL Standard's reading of
   `tauri://localhost` and that reading is wrong on WebKit (see "Platform targets"),
   so `viewers/pdf/pdfjs.ts` now takes URLs out of the question entirely: the worker
   runs from inlined source, it is measured before it is used, *that* worker is the one
   pdf.js gets via `getDocument({ worker })`, and every await on the path has a
   deadline. What has been measured, from a real `tauri://` scheme handler on
   webkit2gtk: the full 28-check suite passes in `worker` mode; with the blob worker
   forced to fail, 27/28 pass in `main-thread` mode and a real PDF still opens; with
   *nothing* able to start the decoder, the tile shows a named, retryable error in
   about fifteen seconds instead of a spinner. What is left is the mode. Open a PDF in
   a macOS **bundle** (not `tauri dev`, which cannot reproduce any of this) and confirm
   the dev self-test says `worker` rather than `main-thread`. If it says `main-thread`
   then PDFs work and the platform is being carried: that is the thing to look at next,
   and it is now visible instead of silent.
